...
import datetime
import os
from langchain_openai import ChatOpenAI  # 使用推荐的导入方式
from langchain.schema import HumanMessage, SystemMessage

# 使用DeepSeek API替代OpenAI
chat = ChatOpenAI(
    model="deepseek-coder",  # 或具体的DeepSeek模型名称
    openai_api_base="https://api.deepseek.com/v1",  # DeepSeek API端点
    openai_api_key="sk-79c827a4693b4bb89189d17b52f22f59",  # 您的DeepSeek API密钥
    temperature=1,
    max_tokens=4095,
    top_p=1,  # 直接作为参数，而不是放在model_kwargs中
    frequency_penalty=0,  # 直接作为参数
    presence_penalty=0  # 直接作为参数
)
...
def main():
    ...
    raw_content_data = []
    with open('/root/autodl-tmp/LLM-quickstart-main/chatglm/data/raw_data.txt', 'r', encoding='utf-8') as file:
        content = file.read()
